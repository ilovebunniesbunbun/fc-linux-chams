...
local function log_debug_info(current_time)
    ...
    table.insert(parts, string.format("  Local Eye: [ %.2f, %.2f, %.2f ]",
    ...
    if local_packet.held_grenade_type > 0 then
        table.insert(parts, string.format("  Held Grenade: Type: %d, Pin: %d, Strength: %.2f, Velocity: [ %.1f, %.1f, %.1f ], Angles: [ %.1f, %.1f ]",
            ...
            local_packet.local_angles.x, local_packet.local_angles.y))
    else
    ...
    for i = 0, local_packet.projectile_count - 1 do
        ...
        table.insert(parts, string.format("    Proj %d: Type: %d, Handle: %u, Init: [%.1f, %.1f, %.1f], Curr: [%.1f, %.1f, %.1f], TimerStart: %.2f, Dur: %.1f",
            i, proj.type, proj.entity_handle,
            ...
            proj.timer_start_time, proj.duration))
    end

    for i = 0, local_packet.inferno_count - 1 do
        local inf = local_packet.infernos[i]
        table.insert(parts, string.format("    Inferno %d: Handle: %u, Start: %.2f, Dur: %.1f, Flames: %d",
            i, inf.entity_handle, inf.start_time, inf.duration, inf.fire_count))
    end
...
function bridge.on_worker(is_calibrated)
    ...
    if bridge.debug and (current_time - last_log_time >= 5000) then
